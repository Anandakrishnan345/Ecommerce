...
//                   <li
//                     key={product._id}
//                     className="bg-white rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-300 ease-in-out overflow-hidden p-8 "
//                   >
...
// export default GetproductasBuyer;
import React, { useState, useEffect } from "react";
import axios from "axios";
import { Link } from "react-router-dom";
import BASE_URL from "../../Api-handler/Baseurl";

const GetproductasBuyer = ({ searchTerm }) => {
  const [productsByCategory, setProductsByCategory] = useState({});
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [favorites, setFavorites] = useState({});
  ...
  const renderStarRating = (rating) => {
    const filledStars = Math.floor(rating); // Use Math.floor to always round down
